...
2. Do all LEDs toggle at *exactly* the same time? [*no because they are on different timers*]
...
1. What has happened to the speed of the timers? [*The timers slowed down*]

2. What is the new frequency of LED D1? [* 0.25Hz *]

3. When we changed the frequency, did the Seven-Segment Light update rate change?  (hint, look at the clocks driving the APB1, APB2 buses and which timers are on which bus.  Recall that the Seven-Segment timer is Tim17) [*yes, because Timer17 is connected to APB2*]
...
If you do any of these items - just mention what and how it worked, [*made it so that the D1-4 lights turned on instead of 8888 on the seven-seg display*].
